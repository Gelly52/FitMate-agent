...
public class PromptTemplateManager {
    ...
    /**
     * 用户记忆提取提示词模板
     * LLM 根据对话内容提取值得长期记住的用户信息（FACT/EPISODIC/INSIGHT），输出严格 JSON
     */
    private static final String MEMORY_EXTRACT_TEMPLATE = """
            # 用户记忆提取
     ...
     * 构建用户记忆提取提示词
     *
     * @param conversation 待提取记忆的对话文本
     * @return 填充后的 prompt
     */
    public String buildMemoryExtractPrompt(String conversation) {
        return MEMORY_EXTRACT_TEMPLATE.replace("{conversation}", conversation == null ? "" : conversation);
    }
    ...
    /**
     * 用户画像生成提示词模板
     * LLM 基于用户的长期记忆（FACT/EPISODIC/SNAPSHOT/INSIGHT）生成简洁用户画像，输出严格 JSON
     */
    private static final String PROFILE_BUILD_TEMPLATE = """
            # 用户画像生成
     ...
     * 构建用户画像生成提示词
     *
     * @param facts      稳定事实（FACT）记忆文本
     * @param episodics  近期事件（EPISODIC）记忆文本
     * @param snapshot   近期状态快照（SNAPSHOT）文本
     * @param insights   洞察（INSIGHT）文本
     * @return 填充后的 prompt
     */
